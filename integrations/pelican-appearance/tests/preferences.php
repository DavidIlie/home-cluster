...
Auth::loginUsingId(2);$enricoTheme=ThemePreference::current();
DB::beginTransaction();
try {
 Auth::loginUsingId(1);ThemePreference::save('nord');
 Auth::loginUsingId(2);check(ThemePreference::current()===$enricoTheme,'David theme change leaves Enrico unchanged');
...
foreach(['nord-theme','catppuccin-mocha-theme','voidwave-theme','neobrutalism-theme'] as $id)check(isset($manifest["plugins/$id/resources/css/theme.css"]),"$id assets built");
